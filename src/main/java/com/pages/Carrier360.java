...
public class Carrier360  extends BasePage {
	...
	By Agency_carrier1 = By.xpath("//*[@href='#/agency/carrier']");
	...
	private By SearchCarriergroup = By.xpath("(//*[@type='search'])[1]");
...
//	public void NavigateToCarrierPage() throws InterruptedException {
//		hoverOnAgencyModule();
//	}
	...
	///Creating Carrier group ///Author :- Asif
	public void CreatingCarrierGroup() throws InterruptedException {
		ClickOnMinimizeCallwindow();
		String CarrierName = "Commission Management";
		String CarrierName1 = "ADVANTICA";
		
		hoverOnAgencyModule();
		element = wait.until(ExpectedConditions.presenceOfElementLocated(Agency_carrier1));
		MoveToElementClick(element);
		waitForElement(2000);
		element = wait.until(ExpectedConditions.presenceOfElementLocated(CarrierGroupButton));
		MoveToElementClick(element);
		...
		waitForElement(5000);
		ClickOnMinimizeCallwindow();
		element = wait.until(ExpectedConditions.presenceOfElementLocated(SearchCarriergroup));
		writeText(SearchCarriergroup,CarrierName + Keys.ENTER );
		waitForElement(4000);
		ClickOnMinimizeCallwindow();		
		if (ElementDisplayed(Select_carrier_group)) {
		...
		element = wait.until(ExpectedConditions.presenceOfElementLocated(CarrierGroupNameField));
		MoveToElementClick(element);		
		writeText(CarrierGroupNameField,CarrierName + Keys.ENTER );
		ExtentSuccessMessage("Carrier Name has been Entered");
		
		By Elements=By.xpath("//*[contains(text(),'"+CarrierName1+"')][@class='container-check group-number ng-binding ng-scope']");
		if(ElementDisplayed(Elements)) {
			Base_Class.ScrollUntilElementVisible(Elements);
			click(Elements);
			ExtentSuccessMessage("Carrier has been Clicked");
		}else {
			Base_Class.ScrollUntilElementVisible(Elements);
			click(Elements);
			ExtentSuccessMessage("Carrier has been Clicked");
		...
		Assert.assertEquals(msg, "Carrier group created successfully.");
	...
	public void existing_carriers_Tabs() throws InterruptedException
	{
		...
		writeText(Search ,  Carrier_Ext+ Keys.ENTER);
		...
		Assert.assertTrue(ExtName.contains(Carrier_Ext));
